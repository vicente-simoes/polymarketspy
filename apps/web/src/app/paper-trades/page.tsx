...
interface CopyAttemptRow {
    ...
    spreadMicrosAtDecision?: number | null
    groupKey: string
    createdAt: string
...
export default function PaperTradesPage() {
    ...
    const getAttemptMeta = (groupKey: string) => {
        ...
        const windowStartMs = match ? Date.parse(match[0]) : NaN
        return {
            side,
            ...
            <div className="h-full overflow-y-auto no-scrollbar">
                <main className="flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6 pt-20 md:pt-24 min-h-full">
                    ...
                    <div className="flex-1 flex flex-col gap-4 md:gap-6 min-w-0">
                        <div>
                            <p className="text-sm text-[#6f6f6f]">Paper Trades</p>
                                ...
                                <div className="md:hidden flex flex-col gap-3">
                                    {filteredAttempts.length > 0 ? (
	                                        filteredAttempts.map((attempt) => {
	                                            ...
	                                            const spreadMicros =
	                                                typeof attempt.spreadMicrosAtDecision === "number"
	                                                    ? attempt.spreadMicrosAtDecision
	                                                    : null
	                                            const spreadCents =
	                                                spreadMicros !== null ? spreadMicros / 10000 : null
	                                            ...
	                                            const isSpreadTooWideSkip =
	                                                attempt.decision === "SKIP" &&
	                                                attempt.reasonCodes.includes("SPREAD_TOO_WIDE") &&
	                                                spreadCents !== null
                                                    ...
                                                    <div className="mt-3 flex items-center justify-between gap-3">
                                                        <div className="min-w-0">
                                                            ...
                                                            <div className="text-xs text-[#6f6f6f] truncate">
                                                                ...
                                                                {attempt.bookSource ? ` · ${attempt.bookSource}` : ""}
                                                            </div>
                                                        </div>
                                                        {side ? (
                                                            <span
                                        ...
                                        <tbody>
	                                            {filteredAttempts.length > 0 ? (
	                                                filteredAttempts.map((attempt) => {
	                                                    ...
	                                                    const slippageLabel =
	                                                        isSpreadTooWideSkip
	                                                            ? `spread ${spreadCents!.toFixed(2)}c`
	                                                            : slippageCents === null
	                                                              ? "--"
                                                        ...
                                                        >
                                                            ...
                                                            <td className="py-4 px-3">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {attempt.bookSource ? (
                                                                        <span
                                                                            className={`rounded-full border border-[#27272A] px-2 py-1 text-[10px] font-semibold ${
                                                                                attempt.bookSource === "REST"
                                                                                    ? "bg-[#111827] text-[#93c5fd]"
                                                                                    : "bg-[#111111] text-[#cfcfcf]"
                                                                            }`}
                                                                        >
                                                                            {attempt.bookSource}
                                                                        </span>
