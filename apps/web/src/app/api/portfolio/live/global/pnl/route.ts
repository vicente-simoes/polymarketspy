...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const payload = await getOrSetServerCache(`portfolio:live:global:pnl:${range}`, 30_000, async () => {
            return withPgStatementTimeout(4000, async (tx) => {
                ...
                const pnlCurve = sampled.map((p) => ({
                    ...
                    value: Number(p.pnlMicros) / 1_000_000,
                }))
        ...
        console.error("Failed to fetch live pnl curve:", error)
